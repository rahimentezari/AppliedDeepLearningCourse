...
class LeNet5(object):
    ...
    def construct_weights(self, initializer_w, initializer_b, trainable, scope):
        ...
        with tf.variable_scope(scope):
            ...
            weights['w4'] = tf.get_variable('w4', [120, 84], **w_params)
            weights['w5'] = tf.get_variable('w5', [84, 10], **w_params)
            weights['b1'] = tf.get_variable('b1', [6], **b_params)
            weights['b2'] = tf.get_variable('b2', [16], **b_params)
            weights['b3'] = tf.get_variable('b3', [120], **b_params)
            weights['b4'] = tf.get_variable('b4', [84], **b_params)
            weights['b5'] = tf.get_variable('b5', [10], **b_params)
        return weights
    ...
    def forward_pass(self, weights, inputs, is_train, trainable=True):
        ...
        conv1 = tf.nn.bias_add(conv1, weights['b1'])
        # Activation.
        ...
        conv2 = tf.nn.bias_add(conv2, weights['b2'])
        # Activation.
        ...
        fl1 = tf.add(tf.matmul(fl0, weights['w3']), weights['b3'])
        # Activation.
        fl1 = tf.nn.relu(fl1)

        # Layer 4: Fully Connected. Input = 120. Output = 84.
        fl2 = tf.add(tf.matmul(fl1, weights['w4']), weights['b4'])
        # Activation.
        fl2 = tf.nn.relu(fl2)

        # Layer 5: Fully Connected. Input = 84. Output = 10.
        logits = tf.add(tf.matmul(fl2, weights['w5']), weights['b5'])
